...
void Mario::setImage()
{
    readimagefile("mr.jpg", x, y, x + size, y + size);
	getimage(x, y, x + size, y + size, image);
    undraw();
}
void Mario::setSpike(Spike* _spike) { spikes[spikecount] = _spike; spikecount++; }
void Mario::setLifebar(Lifebar* _lifebar) { lifebar = _lifebar; }
void Mario::setBlock(Block* _block){ blocks[blockcount] = _block; blockcount++; }
void Mario::setEndpoint(Endpoint*_endpoint) { endpoint = _endpoint; }
void Mario::setCoin(Coin* _coin) { coins[coincount] = _coin; coincount++; }
void Mario::setEnemy(Enemy* _enemy) { enemys[enemycount] = _enemy; enemycount++; }
...
{ 
    ...
    // readimagefile("mrb.jpg", x, y, x + size, y + size);
}
...
{   
    ...
    for (int i = 0; i < SPIKEMAX; i++)
    {
        ...
        {   
            ...
            lifebar->undraw(); 
            lifebar->draw();
            readimagefile("gameover.jpg", 0.2*screenWidth, 0.2*screenHeight, 0.8*screenWidth, 0.8* screenHeight);
        }
    }
}
...
void Mario::endpointCollision()
{
    ...
    {   
        ...
        x = screenWidth + 1; 
        y = screenHeight + 1;
        readimagefile("victory.jpg", 0.2*screenWidth, 0.2*screenHeight, 0.8*screenWidth, 0.8* screenHeight);
    }
